*======================================================================*        
*                                                                               
       SUBROUTINE ZPJD( N, A, JA, IA, EIGS, RES, X, LX, NEIG,                   
...
*              ZPJD/ZPJDREVCOM ASSUMES THAT EACH OFF-DIAGONAL ENTRY IS          
...
*                                                                               
*              ZPJD ASSUMES IN ADDITION THAT ALL DIAGONAL ELEMENTS ARE          
...
*              above, ZPJD assumes that some of these JA(K) (for                
...
*                                                                               
*  X       (input/output+workspace) DOUBLE COMPLEX array, dimension LX.         
...
*              equal to the value on output from ZPJD/ZPJDREVCOM.               
*                                                                               
...
*              memory area that is offered by the user, ZPJD will               
...
*          ICNTL(1) Not used (kept for compatibility with ZPJDREVCOM).          
...
*                   option presumes that ZPJD was called previously and         
...
*                   (other computation remain in double precision).             
*                                                                               
...
*                                                                               
        ...
        IJOB=0                                                                  
*                                                                               
        CALL zpjdrvcom( N, A, JA, IA, B, JB, IB, EIGS, RES, X, LX,              
...
*======================================================================*        
*                                                                               
       SUBROUTINE ZPJDREVCOM( N, A, JA, IA, EIGS, RES, X, LX, NEIG,             
...
*  =========                                                                    
*                                                                               
*    N,A,JA,IA,                        |  see comments in subroutine ZPJD       
*                                                                               
*       In addition:                                                            
*                                                                               
...
*               passed to ZPJDREVCOM is only used to define a                   
...
*                                                                               
*            A,JA,IA should be compliant with the format described in ZPJD      
*            However:                                                           
...
*    ITER, TOL, SHIFT, DROPTOL, MEM,   |  see comments in subroutine ZPJD       
*                                      |                                        
*    IPRINT, INFO, GAP                 |                                        
*                                                                               
*                                                                               
...
*                   started)                                                    
*          other entries in ICNTL: see comments in subroutine ZPJD              
*                                                                               
*                                                                               
...
*                        MATVEC) and return to ZPJDREVCOM leaving IJOB          
...
*======================================================================*        
*                                                                               
       SUBROUTINE ZJDREVCOM( N, EIGS, RES, X, LX, NEIG, SIGMA,                  
...
*    SIGMA, ISEARCH, NINIT,            |                                        
*    MADSPACE, ITER, TOL,              |  see comments in subroutine ZPJD       
*                                      |                                        
*    IPRINT, INFO, GAP                 |                                        
*                                                                               
*                                                                               
...
*                       PSOLVE) and return to ZPJDREVCOM leaving IJOB           
*                       (and other parameters) unchanged.                       
*                                                                               
*  NDX1    (output) INTEGER.                                                    
*  NDX2    Indicate indices into X() for the needed MATVEC when IJOB=1,         
...
*                                                                               
       integer NDX3                                                             
*                                                                               
       CALL zjdrvcom( N, EIGS, RES, X, LX, NEIG, SIGMA,                         
...
** ============================================================                 
     ...
     +                  flagsingle                                              
      ...
      external zdglprecdelete,dheramgdelete                                     
                                                                                
      external cheramgdelete                                                    
                                                                                
                                                                                
c     make sure that ZPJDCLEANUP is not really executed before ZPJD/            
c     ZPJDREVCOM has been called for the first time                             
      if (ilumem.ne.-1) then                                                    
         if (factdgl.gt.0) then                                                 
            call zdglprecdelete(IPdiag)                                         
         else if (factdgl.lt.0) then                                            
                                                                                
            if (flagsingle) then                                                
               call cheramgdelete(IPparam,IPPREC,IPnlev,0)                      
            else                                                                
               call dheramgdelete(IPparam,IPPREC,IPnlev,0)                      
            end if                                                              
                                                                                
                                                                                
                                                                                
         end if                                                                 
c        ensure that twice calling ZPJDCLEANUP is captured                      
         ilumem=-1                                                              
      end if                                                                    
      factdgl=0                                                                 
c                                                                               
...
*======================================================================*        
*                                                                               
       SUBROUTINE ZPJD_GEP( N, A, JA, IA, B, JB, IB, EIGS, RES, X, LX,          
     +                      NEIG, SIGMA, ISEARCH, NINIT, MADSPACE, ITER,        
...
*            storage scheme as for A (see comments in subroutine ZPJD).         
...
*    MADSPACE, ITER,                   |  see comments in subroutine ZPJD       
*                                      |                                        
*    SHIFT, DROPTOL, MEM,              |                                        
...
*======================================================================*        
*                                                                               
       SUBROUTINE ZPJDREVCOM_GEP( N, A, JA, IA, B, JB, IB, EIGS, RES, X,        
...
*               passed to ZPJDREVCOM_GEP is only used to define a               
*               preconditioner, whereas matrix vector multiplications           
*               are performed by a user provided routine via the reverse        
*               communication protocol (see below).                             
*                                                                               
*            A,JA,IA and B,JB,IB  should be compliant with the format           
*            described in ZPJD.However:                                         
*                  zero diagonal entries need not to be referenced in           
*                        the structure (although they may);                     
*                  if a diagonal preconditioning is wanted, one                 
*                        should set ja(1) negative;                             
...
*                       (call to matrix vector multiplication routine:          
*                        MATVEC) and return to ZPJDREVCOM_GEP leaving IJOB      
*                       (and other parameters) unchanged.                       
...
** ============================================================                 
*                                                                               
       CALL zpjdrvcom( N, A, JA, IA, B, JB, IB, EIGS, RES, X, LX, NEIG,         
     +                 SIGMA, ISEARCH, NINIT, MADSPACE, ITER, TOL,              
...
*======================================================================*        
*                                                                               
       SUBROUTINE ZJDREVCOM_GEP(N, EIGS, RES, X, LX, NEIG, SIGMA,               
     +                          ISEARCH, NINIT, MADSPACE, ITER, TOL,            
...
*               IJOB=2: solve Prec*X(NDX1:NDX1+N-1) = X(NDX2:NDX2+N-1)          
*                       (call to preconditioner solve routine:                  
*                       PSOLVE) and return to ZPJDREVCOM_GEP leaving IJOB       
*                       (and other parameters) unchanged.                       
*               IJOB=3: compute X(NDX2:NDX2+N-1)= A*X(NDX1:NDX1+N-1)            
*                       (call to matrix vector multiplication routine:          
*                        MATVEC) and return to ZPJDREVCOM_GEP leaving IJOB      
