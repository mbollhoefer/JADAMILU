...

{
    ...
    mwSize     buflen, mrows, ncols, ndim, mydims[2], *A_ia,*A_ja;
    ...
    integer    *ia, *ja, maxeig, ninit, iprint, isearch, iter, madspace, disp,
    ...
    a =(doublecomplex *)mxCalloc((size_t)nnz,  (size_t)sizeof(doublecomplex));
    l=0;
    ia[0]=1;
    for (i=0; i<n; i++) {
        for (j=A_ia[i]; j<A_ia[i+1]; j++) {
    ...
    mexPrintf("call PJD\n"); fflush(stdout);
#endif
    zpjd(&n,a,ja,ia, eigs, res, X, &lx, &neig, &sigma, &isearch, &ninit, 
