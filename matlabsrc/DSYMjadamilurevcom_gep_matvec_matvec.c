...
/*
    ...
    [V,D,options] = DSYMjadamilurevcom_gep_matvec_matvec(ANAME,n,MNAME,k,sigma,options,PREC);
...

{
    ...
    mxArray    *tmp, *fout, *aname_input, *n_input, *mname_input, *options_input, *options_output,
               *precname_input, *k_input, *sigma_input, *V_output, *D_output, *V0;
    char       *pdata, *input_buf, *output_buf, *aname, *mname, *precname;
    ...
    mxArray    *lhs[1], *a_m_prec_rhs, *fname_output;
    ...
    /* The third input must be a function name of M */
    mname_input=(mxArray *)prhs[2];
    if (!mxIsChar(mname_input)) 
       mexErrMsgTxt("Third input must be a string");
    buflen=(mxGetM(mname_input)*mxGetN(mname_input))+1;
    /* Allocate memory for input string. */
    mname=(char *)mxCalloc((size_t)buflen, (size_t)sizeof(char));
    mxGetString(mname_input, mname, buflen);
#ifdef PRINT_INFO
    mexPrintf("function M name: %s\n",mname); fflush(stdout);
#endif    
    ...
    if (!mxIsStruct(options_input)) 
       mexErrMsgTxt("Sixth input must be a structure");
    ...
    mydims[0]=n;
    mydims[1]=1;
    a_m_prec_rhs=(mxArray *)mxCreateNumericArray(2, mydims, mxDOUBLE_CLASS, mxREAL);
    do {
#ifdef PRINT_INFO
       mexPrintf("call JDREVCOM\n"); fflush(stdout);
#endif
       djdrevcom_gep(&n,eigs, res, X, &lx, &neig, &sigma, &isearch, &ninit, 
		     &madspace, &iter, &tol, &ijob, &ndx1, &ndx2, &ndx3, 
		     &iprint, &info, &gap);
	  ...
	  if (ijob==1 || ijob==3) {
	     ...
	     /* copy pin -> a_m_prec_rhs */
	     pr=mxGetPr(a_m_prec_rhs);
	     for (i=0; i<n; i++)
	         pr[i]=pin[i];

	     mexCallMATLAB(1,lhs,1,&a_m_prec_rhs, aname);
	  ...
	  if (ijob==1 || ijob==4) {
	     ...
	     mexCallMATLAB(1,lhs,1,&a_m_prec_rhs, mname);
	  ...
	  mexCallMATLAB(1,lhs,1,&a_m_prec_rhs, precname);
